...
export type SfxTiming = 'start' | 'middle' | 'end';
...
export type AudioScene = {
  ...
  sfx: { type: SfxType; at: SfxTiming; volume: number };
};
...
export function createAudioPlan(input: {
  ...
  const scenes: AudioScene[] = input.scenes.map((scene, index) => {
    ...
    const at: SfxTiming = purpose.includes('cta') ? 'end' : 'start';
    return {
      ...
      sfx: { type: sfx, at, volume: sfx === 'impact' ? 0.42 : 0.24 }
    };
  });
