...
export async function mixAudio(videoPath:string,tracks:AudioTracks,output='./tmp/render/final.mp4'){
 ...
 if(labels.length===1)filters.push(`${labels[0]}anull[aout]`);else filters.push(`${labels.join('')}amix=inputs=${labels.length}:duration=longest:dropout_transition=2[aout]`);
