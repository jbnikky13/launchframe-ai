...
/** Original, dependency-free SVG puppet frames for the LaunchFrame renderer. */
export function puppetSvg({ action = 'idle', emotion = 'neutral', phase = 0, width = 420, height = 620 } : { action?: PuppetAction; emotion?: PuppetEmotion; phase?: number; width?: number; height?: number } = {}) {
  ...
  const bob = action === 'walk' ? Math.abs(swing) * 8 : Math.sin(phase * Math.PI * 2) * 2;
  const head = 105 + bob, top = 164 + bob, bottom = 365 + bob;
  const talk = action === 'talk' && Math.floor(phase * 10) % 3 !== 0;
  let la = `M ${x} ${top + 45} L ${x - 88} ${top + 120}`, ra = `M ${x} ${top + 45} L ${x + 88} ${top + 120}`;
  let ll = `M ${x} ${bottom} L ${x - 68} 505`, rl = `M ${x} ${bottom} L ${x + 68} 505`;
  ...
  const mouth = talk ? `<ellipse cx="${x}" cy="${head + 28}" rx="16" ry="11" fill="#111827" stroke="none"/>` : `<path d="M ${x - 20} ${head + 27} Q ${x} ${head + (emotion === 'happy' ? 43 : 35)} ${x + 20} ${head + 27}"/>`;
...
export function puppetActionForStickman(action?: string): PuppetAction {
  if (action === 'pointing') return 'pointing';
  if (action === 'celebrating') return 'celebrating';
  if (action === 'thinking') return 'thinking';
  if (action === 'confused') return 'confused';
  return 'walk';
}
