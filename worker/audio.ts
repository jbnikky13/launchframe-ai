...
export async function mixAudio(videoPath:string,tracks:AudioTracks,output='./tmp/render/final.mp4'){
 ...
 const filters:string[]=[];const labels:string[]=[];let i=1;if(tracks.voiceoverPath){filters.push(`[${i}:a]volume=${tracks.voiceVolume??1}[voice]`);labels.push('[voice]');i++;}if(tracks.musicPath){filters.push(`[${i}:a]volume=${tracks.musicVolume??0.16}[music]`);labels.push('[music]');}
 ...
 await execFileAsync('ffmpeg',[...inputs,'-filter_complex',filters.join(';'),'-map','0:v:0','-map','[aout]','-c:v','copy','-c:a','aac','-b:a','192k','-movflags','+faststart',output],{maxBuffer:1024*1024});return output;
}
