...
export function normalizeAssets(input:unknown):ProjectAsset[]{
 ...
 return input.filter((x):x is Record<string,unknown>=>!!x&&typeof x==='object').map((x,i):ProjectAsset=>({
  id:typeof x.id==='string'?x.id:`asset-${i+1}`,
  name:typeof x.name==='string'?x.name:`Screenshot ${i+1}`,
  url:typeof x.url==='string'?x.url:'',
  kind:x.kind==='image'?'image':x.kind==='website_capture'?'website_capture':'screenshot',
  mimeType:typeof x.mimeType==='string'?x.mimeType:undefined,
  selected:true
 })).filter((a):a is ProjectAsset=>Boolean(a.url));
}
