...
function noStore(body: unknown, init?: ResponseInit) {
  return NextResponse.json(body, { ...init, headers: { ...(init?.headers || {}), 'Cache-Control': 'no-store, max-age=0, must-revalidate' } });
}
...
export async function GET(_: Request, { params }: { params: { id: string } }) {
  ...
  if (!data) return noStore({ id: params.id, status: 'not_found', progress: 0, error: 'This render job is no longer available.' });
...
export async function DELETE(_: Request, { params }: { params: { id: string } }) {
  ...
  // Clean the rendered object first. If storage cleanup fails, keep the DB row
  // so the user can retry instead of creating an orphaned MP4.
  const { error: storageError } = await db.storage.from('launchframe-renders').remove([`renders/${id}.mp4`]);
  if (storageError) return noStore({ error: `Unable to delete rendered file: ${storageError.message}` }, { status: 500 });
  ...
  return noStore({ ok: true, id, deleted: true, storageCleanupPending: false });
}
