...
function inferCharacterAction(purpose:string,index:number,puppetTemplate=false):StickAction|null {const value=purpose.toLowerCase();if(value.includes('problem')||value.includes('pain')||value.includes('conflict'))return'confused';if(value.includes('feature')||value.includes('show')||value.includes('point'))return'talking';if(value.includes('benefit')||value.includes('result')||value.includes('success'))return'celebrating';if(value.includes('think')||value.includes('question'))return'thinking';if(value.includes('hook')||value.includes('intro'))return index%2===0?'talking':'pointing';return puppetTemplate?'talking':null;}
